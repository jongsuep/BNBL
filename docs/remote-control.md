...
1. **Install the Claude mobile app**: Download the Claude app for [iOS](https://apps.apple.com/us/app/claude-by-anthropic/id6473753684) or [Android](https://play.google.com/store/apps/details?id=com.anthropic.claude).
2. **Sign in with your Claude Code account**: Use the same account and organization you use for Claude Code in the terminal.
3. **Allow notifications**: Accept the notification permission prompt from the operating system.
4. **Enable push in Claude Code**: In your terminal, run `/config` and enable **Push when Claude decides**.
